...
### 16.1.1 자율 정리 사이클 (2026-04-29 ~ 2026-04-30 / 페이즈 1–12)
...
**누적 효과**: 회귀 테스트 37건 신규 (백엔드 103건 + 프론트 unit 51건 = 총 154건 통과), 보안 패치 4건, 잠재 버그 2건, UX/i18n 정리 4건, 워크플로 막힘 해소 1건, 신규 페이지 1건. `make doctor` 신규 회귀 없음.
...
| 4 | UX(데이터 일관성) | **시드 상품의 기본 status가 "비활성"**: `Product.status`의 default가 `"draft"`/`"INACTIVE"`로 추정. 사용자가 상품 등록 시 active로 바로 시작하도록 default 또는 등록 폼 토글. | `apps/api/src/api/v1/products.py`, `apps/api/src/domain/product/entities.py` |
