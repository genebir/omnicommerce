...
- [x] **Recent Activity i18n + 상대 시간 로케일 (페이즈 11)**: `/dashboard/activity` API에 `title_key`+`params` 필드 추가(한국어 `description`은 fallback 유지), `RecentActivity` 컴포넌트에서 `t(...)` 렌더. `formatRelative(date, locale)`로 시그니처 확장 + `Intl.RelativeTimeFormat` 채택. 영어 모드에서 활동 텍스트와 상대 시간이 한국어로 새던 회귀 차단
...
### 16.1.1 자율 정리 사이클 (2026-04-29 ~ 2026-04-30 / 페이즈 1–19)
...
- [x] **페이즈 15 — i18n 후속(채널 브랜드명·Topbar 알림·ConnectWizard·줄바꿈)**: 영어 모드에서 한글이 새던 4곳 정리. ① **채널 카드 제목** — 백엔드 `channel_types.name`이 "카페24/네이버 스마트스토어/쿠팡" 한글 단일 컬럼이라 영어 모드에서도 한글로 노출됐음. `channels.brandName{Cafe24,Naver,Coupang}` 키 추가 + `ChannelCard`에서 `useMessages()`로 카탈로그 존재 여부 확인 후 `t(...)` 우선, 없으면 prop 폴백. ② **카드 줄바꿈** — `<h3>`에 `truncate` 대신 `break-words` + `min-w-0` + `gap-3` 부모로 변경, 영어 "Naver Smart Store"가 두 줄로 자연 분리되어 카드 높이 일관성 유지. ③ **ConnectWizard RedirectUriPanel** — `Redirect URI 미설정`/긴 본문 hint/`설정 방법 보기` 한글 하드코딩 3개를 `tunnelMissingTitle`/`tunnelMissingBodyPrefix`/`tunnelSetupGuide` 키로 분리. ④ **Topbar 알림 데모 데이터** — 임시 모킹이지만 영어 모드 노출이라 `topbar.demoNotif{1,2}{Title,Body}` 4키 추가, `t()` 호출. 회귀 테스트 `messages.test.ts` 1건(페이즈 15 키 10개 동기성). 라이브 검증: `/channels` 페이지 영어 모드 캡처에서 "Cafe24/Naver Smart Store/Coupang" 정확히 노출, 카드 높이 일관. NB — Storybook 스위트의 `useQueryClient` 미주입 회귀(8개 스토리 실패)는 페이즈 15 이전부터 깨져 있던 무관한 인프라 문제, 후속 페이즈로 분리.
- [x] **페이즈 16 — 부트스트랩 관리자 시드(첫 admin 운영 가능 상태로 만듦)**: 페이즈 8에서 `/admin/settings/*` 게이트를 `is_superuser=True`로 막은 뒤 정작 관리자를 만드는 공식 경로가 없어, 신규 셋업 머신에서 누구도 게이트를 통과 못 하던 운영 막힘. `apps/api/scripts/seed_admin.py` 신규 — `BOOTSTRAP_ADMIN_EMAIL`/`BOOTSTRAP_ADMIN_PASSWORD` 환경변수가 설정되어 있으면 `is_superuser=True` 사용자를 멱등 생성. 안전 가드: ① 비어있는 패스워드 → skip(`skipped_no_password`) ② 동일 이메일 admin 이미 존재 → skip(`skipped_already_admin`) ③ 동일 이메일 일반 사용자 점유 → 자동 승격 안 함, 비밀번호 절대 덮어쓰지 않음(`skipped_existing_user`). `setup.py`에 단계 [6/6] 추가, `make seed`/`setup.py --seed`/`setup.py`(전체)에서 자동 호출. `.env.example`의 `BOOTSTRAP_ADMIN_PASSWORD` 기본값을 placeholder에서 빈 문자열로 변경(fail-safe — 명시적 설정 없으면 admin 생성 안 됨) + 사용 안내 주석 추가. 회귀 테스트 `tests/integration/test_admin_seed.py` 4건 (빈 패스워드 skip / 신규 생성 / 멱등 / 일반 사용자 보호).
- [x] **페이즈 17 — 프론트엔드 admin 가드(사이드바 + 페이지)**: 백엔드는 페이즈 8에서 403으로 차단되지만 프론트엔드는 일반 셀러에게도 사이드바 "관리자 설정" 항목과 `/admin/settings` 페이지가 노출되어, 클릭 시 백엔드 403 토스트가 뜨는 어색한 흐름이었음. 페이즈 16에서 admin 사용자 생성이 가능해졌으므로 이제 의미 있는 가드. 변경: ① `UserResponse`에 `is_superuser: bool` 필드 추가, `/auth/register`·`/auth/me` 응답에 노출. ② `auth` 스토어 `User` 인터페이스에 `is_superuser?: boolean` 추가. ③ `Sidebar.tsx` — `NavItem.adminOnly` 플래그 도입, `is_superuser=false`면 navItems에서 필터링(가시성 자체가 권한 모델의 일부). ④ `components/auth/AdminOnlyGate.tsx` 신규 — 클라이언트 가드, `(app)/admin/settings/page.tsx`에서 `AdminSettingsContent`를 감쌈. 비-admin은 ShieldAlert 안내 카드 + "대시보드로 돌아가기" 버튼. hydrate 전 깜빡임 방지 스켈레톤. ⑤ i18n: `admin.noAccessTitle`/`noAccessDesc`/`backToDashboard` 3키 ko/en. 회귀 테스트 `test_admin_auth.py`에 1건 추가 — `/auth/me`가 일반 사용자/admin 모두에게 `is_superuser` 필드를 정확히 노출하는지 검증(프론트 가드의 의존 계약). 라이브 검증: 일반 셀러로 로그인 후 `/dashboard` 사이드바에 "관리자 설정" 미노출(6개 항목만), `/admin/settings` 직접 URL 입력 시 "관리자 권한이 필요합니다" 안내 페이지로 전환.
- [x] **페이즈 18 — 쿠팡 WING API 코드↔공식 docs 정합성 4건 수정 (mock 회귀 강화)**: 사용자가 사업자 인증 미보유로 라이브 호출 검증 불가지만, 공식 문서(developers.coupangcorp.com) 검증으로 발견한 4건 불일치를 코드와 contract test mock에 반영. 사업자 인증 후 즉시 라이브 호출 가능한 상태로 만듦. ① **재고 갱신 경로 버그** — `update_inventory()`가 `PUT /vendor-items/quantities` + body 일괄 형태였으나 공식엔 단건 path-param `PUT /vendor-items/{vendorItemId}/quantities/{qty}` (body 없음)만 존재. 호출 시 404/405 발생할 회귀였음. 단건 경로로 변경, 다중 SKU 갱신은 호출자 루프 책임으로 NB. ② **주문 조회 누락 파라미터** — `fetch_orders()`가 `createdAtFrom`+`maxPerPage`만 송신했으나 공식은 `createdAtTo` + `searchType=timeFrame` 필수. 빈 응답/4xx 가능했음. `until or datetime.now(UTC)` 기본값 + 4개 파라미터 모두 송신. 24h 초과 청킹은 후속 페이즈로 NB. ③ **statusName ACTIVE 화이트리스트 협소** — `"승인완료"` 단일 매칭이라 `"부분승인완료"`/`"판매중"` 등 노출 가능 상태도 INACTIVE로 잘못 분류. `_ACTIVE_STATUS_NAMES = frozenset({"승인완료", "부분승인완료", "판매중"})` 화이트리스트로 확장. ④ **쿼리 정렬 일치 보장** — `client.py`가 서명용은 `sorted(params)`로 만들지만 httpx 송신 시 `params=dict`로 넘기면 dict 순서로 직렬화돼 서명/송신 query가 달라질 위험. `urllib.parse.urlencode(sorted_items)`로 직접 URL에 query를 박는 방식으로 변경 — 서명과 송신이 반드시 동일 보장. 회귀 테스트 `tests/channels/coupang/test_docs_compat.py` 12건 — 단건 path-param URL/body 없음 검증 + 주문 4개 파라미터 송신 검증 + 화이트리스트 9 파라미터화 케이스 + URL query 알파벳 정렬 검증. 한계: mock 기반이라 "코드 ↔ docs 일치"만 보장, "실제 동작"은 사업자 인증 후 라이브 검증 필요. cafe24·naver 동일 docs 검증 사이클은 §16.2.1 우선순위 1로 분리.
- [x] **페이즈 19 — `start.sh`/`stop.sh` 플랫폼 호환성 강화**: macOS·Linux·WSL2까지만 지원하던 부트스트랩 스크립트가 일부 환경에서 깨지던 케이스를 정리. ① **`docker compose` v1/v2 자동 감지** — `detect_compose()` 헬퍼로 v2(`docker compose`) 우선, 없으면 v1(`docker-compose`) 폴백. 둘 다 없으면 명시적 에러. ② **포트 충돌 검사 다단 폴백** — `lsof` 우선, 없으면 `ss` → `netstat` → `/proc/net/tcp`(awk + hex 포트 매칭) 순. 모두 없으면 검사 자체를 건너뜀(non-fatal). Alpine·minimal Linux·일부 컨테이너 환경 대응. ③ **Linux `PNPM_HOME` 보강** — 기존엔 macOS만 `~/Library/pnpm` 처리했으나 Linux는 누락. `~/.local/share/pnpm`(pnpm setup 기본값)을 PATH에 추가. ④ **OS 분기 확장** — 기존 `Darwin`/(implicit Linux)에 `MINGW*`/`MSYS*`/`CYGWIN*` 케이스 추가. Git Bash on Windows에서 진단 메시지(Docker Desktop 안내) + PATH 보강. ⑤ **`docker compose --wait` 플래그 폴백** — Compose v2.1.1+ 기능이라 구버전엔 없음. `--help` 출력에서 플래그 존재 여부 확인 후 미지원 시 60초 healthcheck 폴링으로 대체. ⑥ **NO_COLOR 환경변수 + 비-tty 색상 비활성화** — CI·로그 캡처 환경에서 escape 코드 노이즈 제거. ⑦ **`stop.sh` 동일 변경** + PIDFILE에 `DOCKER_COMPOSE` 기록해 stop 시점에도 같은 컴포즈 명령 사용. 검증: `bash -n` syntax 통과, `detect_compose()` 단위 검증, `/proc/net/tcp` 폴백 단위 검증(포트 8000 발견). Windows 네이티브 PowerShell(.ps1)은 별도 페이즈로 분리.

**누적 효과**: 회귀 테스트 56건 신규 (백엔드 121건 + 프론트 unit 52건 = 총 173건 통과), 보안 패치 6건, 잠재 버그 2건, **외부 API 정합성 패치 4건(쿠팡)**, **부트스트랩 스크립트 호환성 7건**, UX/i18n 정리 7건, 워크플로 막힘 해소 1건, 진입 막힘 해소 1건, 신규 페이지 1건. `make doctor` 신규 회귀 없음.
...
| 우선 | 영역 | 작업 | 산출 위치 |
|---|---|---|---|
| 1 | 채널 정합성 (cafe24/naver) | 쿠팡과 동일한 docs 검증 사이클을 cafe24·naver에 반복(쿠팡은 페이즈 18에서 정리됨). cafe24는 페이즈 5에서 mock 기반 구현했지만 실제 API 검증 안 됐고, 헤더 `X-Cafe24-Api-Version: 2026-03-01`처럼 의심 가는 부분 있음(미래 날짜). naver 커머스 API는 OAuth2 Client Credentials + 전자서명 인증인데 우리 구현이 그 스펙을 따르는지 확인 필요. | `apps/api/src/infra/channels/{cafe24,naver}/`, 각 `tests/channels/`도 |
| 2 | 쿠팡 청킹 후속 | 페이즈 18에서 NB로 남긴 쿠팡 주문 조회 청킹 — `searchType=timeFrame`은 24h 이내만 허용. 호출자가 1일 초과 범위로 호출하면 422. `services/`나 ARQ 태스크에서 24h 청크로 자동 분할해 gateway를 여러 번 호출하는 헬퍼 추가 필요. | `apps/api/src/infra/queue/tasks.py`, `apps/api/src/infra/channels/coupang/gateway.py` |
| 3 | 테스트 인프라 | **Storybook 스토리들이 `useQueryClient` 등 hook을 부르는 컴포넌트에서 "No QueryClient set"으로 실패** — `LoginForm`/`SignupForm`/`OrderDetail`/`ProductDetail`/`ConnectWizard`/`AdminSettingsContent`/`CommandPalette` 8개 스토리. `.storybook/preview.tsx`에 `QueryClientProvider` + `NuqsAdapter` 글로벌 데코레이터 추가 또는 스토리별 wrapper 도입. 페이즈 15 이전부터 깨져 있던 회귀(이번 페이즈로 발견·격리). | `apps/web/.storybook/preview.tsx`, 각 스토리 파일 |
