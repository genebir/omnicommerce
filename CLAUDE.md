...
### 16.1.1 자율 정리 사이클 (2026-04-29 ~ 2026-04-30 / 페이즈 1–17)
...
**누적 효과**: 회귀 테스트 44건 신규 (백엔드 109건 + 프론트 unit 52건 = 총 161건 통과), 보안 패치 6건, 잠재 버그 2건, UX/i18n 정리 7건, 워크플로 막힘 해소 1건, 진입 막힘 해소 1건, 신규 페이지 1건. `make doctor` 신규 회귀 없음.
...
| 우선 | 영역 | 작업 | 산출 위치 |
|---|---|---|---|
| 1 | 채널 정합성 (쿠팡) | **쿠팡 WING API 코드 ↔ 공식 docs 불일치 4건** (2026-04-30 발견, 라이브 검증은 사업자 인증 후 별도 사이클): (a) **재고 갱신 경로 버그** — 현재 `PUT /vendor-items/quantities` + body `[{vendorItemId, quantity}]`(일괄). 공식은 `PUT /vendor-items/{vendorItemId}/quantities/{quantity}`(단건 path-param) — 호출 시 404/405. (b) **주문 조회 누락 파라미터** — `createdAtTo` + `searchType=timeFrame` 필수인데 우리는 `createdAtFrom`+`maxPerPage`만 보냄. 빈 응답/4xx 가능. (c) **`statusName` ACTIVE 화이트리스트 협소** — `"승인완료"` 단일 매칭, `"부분승인완료"`/`"판매중"` 등 다른 활성 상태도 INACTIVE로 잘못 분류. (d) **쿼리 정렬 위험** — 서명용 `sorted(params)` query와 httpx 실제 직렬화 순서가 다르면 401. 명시적으로 동일 query로 보내야 함. ✓ 일치 항목: 베이스 URL `api-gateway.coupang.com`, HMAC 시각/Authorization prefix(`CEA`), 상품 목록/등록/수정/삭제 경로, 주문 상태 enum 매핑 9종. 검증 출처: developers.coupangcorp.com 공식 문서(WebFetch). | `apps/api/src/infra/channels/coupang/{client,gateway,mapping}.py`, `apps/api/tests/channels/coupang/test_gateway_contract.py` |
| 2 | 채널 정합성 (cafe24/naver) | 쿠팡과 동일한 docs 검증 사이클을 cafe24·naver에 반복. cafe24는 페이즈 5에서 mock 기반 구현했지만 실제 API 검증 안 됐고, 헤더 `X-Cafe24-Api-Version: 2026-03-01`처럼 의심 가는 부분 있음(미래 날짜). naver 커머스 API는 OAuth2 Client Credentials + 전자서명 인증인데 우리 구현이 그 스펙을 따르는지 확인 필요. | `apps/api/src/infra/channels/{cafe24,naver}/`, 각 `tests/channels/`도 |
