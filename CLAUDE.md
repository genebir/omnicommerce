...
| 우선 | 영역 | 작업 | 산출 위치 |
|---|---|---|---|
| 1 | 테스트 인프라 | **Storybook 스토리들이 `useQueryClient` 등 hook을 부르는 컴포넌트에서 "No QueryClient set"으로 실패** — `LoginForm`/`SignupForm`/`OrderDetail`/`ProductDetail`/`ConnectWizard`/`AdminSettingsContent`/`CommandPalette` 8개 스토리. `.storybook/preview.tsx`에 `QueryClientProvider` + `NuqsAdapter` 글로벌 데코레이터 추가 또는 스토리별 wrapper 도입. 페이즈 15 이전부터 깨져 있던 회귀(이번 페이즈로 발견·격리). | `apps/web/.storybook/preview.tsx`, 각 스토리 파일 |
| 2 | 도메인 일관성 | **`domain/product/entities.py`의 status가 소문자**(`"draft"`/`activate()` → `"active"`)인데 DB 모델·채널 매핑·API 응답은 대문자(`"ACTIVE"`/`"INACTIVE"`). 도메인 엔티티 unit 테스트에서만 사용되어 운영 영향은 없으나 일관성 깨짐 — 도메인을 대문자로 통일. | `apps/api/src/domain/product/entities.py`, `apps/api/tests/unit/test_product_entity.py` |
| 3 | 개발자 경험 | (a) `Makefile:doctor`가 `python --version`을 호출하지만 일부 머신엔 `python` 심볼릭 링크가 없음(우분투 표준) — `python3` 변경 또는 `command -v` fallback. (b) `pnpm lint`가 `storybook-static`·`.next`까지 검사해 12000+ warning. `eslint.config.*`에 `ignores` 추가. | `Makefile`, `apps/web/eslint.config.*` |
| 4 | 보안 audit (마무리) | (a) admin 시드 검증 가이드 — `BOOTSTRAP_ADMIN_PASSWORD`를 .env에 넣고 `make seed` 한 번 돌려 첫 관리자 만드는 운영 가이드 README/16.3에 추가. (b) refresh token rotation reuse detection (페이즈 8 NB) — 현재 미구현, 토큰 탈취 시 방어 필요. | `README.md` 또는 `CLAUDE.md §16.3`, `apps/api/src/services/auth_service.py` |
