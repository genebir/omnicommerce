...
### 16.1.1 자율 정리 사이클 (2026-04-29 ~ 2026-04-30 / 페이즈 1–16)
...
**누적 효과**: 회귀 테스트 43건 신규 (백엔드 108건 + 프론트 unit 52건 = 총 160건 통과), 보안 패치 5건, 잠재 버그 2건, UX/i18n 정리 6건, 워크플로 막힘 해소 1건, 진입 막힘 해소 1건, 신규 페이지 1건. `make doctor` 신규 회귀 없음.
...
| 우선 | 영역 | 작업 | 산출 위치 |
|---|---|---|---|
| 1 | 보안 audit (후속) | **프론트엔드 `(app)/admin/settings` 페이지에 `is_superuser` 가드 추가** — 백엔드는 페이즈 8에서 403으로 차단되지만, 프론트엔드는 일반 사용자에게도 사이드바 메뉴(`Admin Settings`)와 페이지가 노출되어 클릭 시 403 토스트가 뜨는 어색한 흐름. `is_superuser=False`면 사이드바 항목 숨김 + 직접 접근 시 안내 페이지(또는 403 폴백). 페이즈 16에서 admin 사용자 자체를 만들 수 있게 됐으므로 이제 의미 있음. | `apps/web/components/layout/Sidebar.tsx`, `apps/web/app/(app)/admin/settings/page.tsx`, `apps/web/stores/auth.ts` |
| 2 | 테스트 인프라 | **Storybook 스토리들이 `useQueryClient` 등 hook을 부르는 컴포넌트에서 "No QueryClient set"으로 실패** — `LoginForm`/`SignupForm`/`OrderDetail`/`ProductDetail`/`ConnectWizard`/`AdminSettingsContent`/`CommandPalette` 8개 스토리. `.storybook/preview.tsx`에 `QueryClientProvider` + `NuqsAdapter` 글로벌 데코레이터 추가 또는 스토리별 wrapper 도입. 페이즈 15 이전부터 깨져 있던 회귀(이번 페이즈로 발견·격리). | `apps/web/.storybook/preview.tsx`, 각 스토리 파일 |
